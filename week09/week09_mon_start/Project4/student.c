...
STUDENT CreateStudent(char Name[], int StudentNum) {
	...
	newStudent.name = (char*)malloc(len);
	if (newStudent.name == NULL) {
		fprintf(stderr, "eror allocating memory\n");
		exit(EXIT_FAILURE);
	}
	memset(newStudent.name, 0, len);

	strncpy(newStudent.name, Name, strlen(Name));	// could use len, but this felt marginally better
